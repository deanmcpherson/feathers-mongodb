...
var toObjectID = function(id) {
  if (typeof id === 'string') return mongo.helper.toObjectID(id);
  return id;
} 
...
var MongoService = Proto.extend({
  ...
  find: function(params, cb) {
    ...
    if (params.query && params.query._id && params.query._id.$in) {
      params.query._id.$in = params.query._id.$in.map(toObjectID);
    }
  ...
  remove: function(id, params, cb) {
    ...
    var getParams = _.assign({}, params, {internal: true});
    
    if (typeof id === 'string') {
      id = toObjectID(id);
    }

    this.get(id, getParams, function(error, data) {
      if(error) {
        return cb(error);
      }
